...
namespace ckm {
    ...
    struct keyframe
    {
        ...
        keyframe(const property_type& property, float t) :
            prop(property), time(t) {}
        keyframe(): prop(), time(0.f) {}
    };

    template<typename _property> bool operator<(const keyframe<_property>& a,
                                                const keyframe<_property>& b)
    {
