...
namespace ckm {
    ...
    void equation<_property>::calc(property_type& out,
                                   const keyframe_type& left,
                                   const keyframe_type& right,
                                   CKTime time) const
    {
        if (time < left.time)
        {
        ...
        CKTimeDelta timeRange = right.time - left.time;
        CKTimeDelta timeIn = time - left.time;
        if (timeRange < timeIn)
            timeIn = timeRange;
    ...
    template<typename _property>
    void tweenProperty(_property& out,
                       const keyframe<_property>& left,
                       const keyframe<_property>& right,
                       CKTime scalar)
    {
